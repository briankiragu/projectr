...
const Project: Component = () => {
  ...
  return (
    <div class="flex flex-col gap-4 items-stretch h-dvh p-6 bg-gray-100">
      ...
      {/* Lyrics */}
      <div
        class="m-6 md:m-10 bg-contain bg-center bg-no-repeat bg-[url('/images/tvc-logo.svg')] opacity-100 flex-auto flex items-center flex-col justify-start gap-4 rounded-lg px-6 py-4 text-center text-tvc-orange transition-colors"
      ...
      >
        <Show
          ...
          <For each={currentVerse()}>
            {(line) => (
              <p class="text-wrap font-extrabold uppercase text-2xl md:text-4xl lg:text-6xl xl:text-7xl 2xl:text-7xl">
                {line}
              </p>
            )}
          </For>
