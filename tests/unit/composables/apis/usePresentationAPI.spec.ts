...
(global as Record<string, unknown>).PresentationRequest = MockPresentationRequest;
...
describe("usePresentationAPI", () => {
  ...
  describe("getAvailability", () => {
    ...
    test("it calls callback with true when availability check fails", async () => {
      ...
      vi.resetModules();
      const { default: freshUsePresentationAPI } = await import(
        "@composables/apis/usePresentationAPI"
      );
